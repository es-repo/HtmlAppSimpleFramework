...
var Renderer = (function () {
    function Renderer(graphicDevice) {
        this.graphicDevice = graphicDevice;
    }
    Renderer.prototype.projectScene = function (scene) {
        var viewMatrix = BABYLON.Matrix.LookAtLH(scene.camera.position, scene.camera.direction, scene.camera.up);
        var projectionMatrix = BABYLON.Matrix.PerspectiveFovLH(scene.camera.fov, this.graphicDevice.get_workingWidth() / this.graphicDevice.get_workingHeight(), scene.camera.zNear, scene.camera.zFar);
    ...
    Renderer.prototype.projectVector = function (v, transMat) {
    ...
    Renderer.prototype.projectVertex = function (vertex, transMat, worldMat) {
    ...
    Renderer.prototype.projectFigure = function (f, worldMatrix, transformMatrix) {
    ...
    Renderer.prototype.projectMesh = function (m, worldMatrix, transformMatrix) {
    ...
    Renderer.prototype.renderMesh = function (m) {
        for (var indexFaces = 0; indexFaces < m.faces.length; indexFaces++) {
            var currentFace = m.faces[indexFaces];
    ...
    Renderer.prototype.renderTriangle = function (v1, v2, v3, color, texture) {
    ...
    Renderer.prototype.processScanLine = function (data, va, vb, vc, vd, color, texture) {
        ...
        for (var x = sx; x < ex; x++) {
            ...
            this.graphicDevice.drawPoint(x, data.currentY, z, new BABYLON.Color4(color.r * ndotl * textureColor.r, color.g * ndotl * textureColor.g, color.b * ndotl * textureColor.b, 1));
        }
    };
